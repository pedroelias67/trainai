...
export async function POST(req: NextRequest) {
  try {
    ...
    // Optional: ?week=5 to target a specific week; otherwise picks first missing
    const url = new URL(req.url);
    const targetWeek = url.searchParams.get("week") ? parseInt(url.searchParams.get("week")!) : null;
    ...
    if (missingWeeks.length === 0) {
      return NextResponse.json({ message: "Plano já tem todas as semanas", added: 0, missing: [] });
    }

    // Generate only 1 week per call to avoid timeout
    const weekToGenerate = targetWeek && missingWeeks.includes(targetWeek) ? targetWeek : missingWeeks[0];
    ...
    // Find the generated week matching weekToGenerate
    const week = planData.weeks.find((w: any) => w.weekNumber === weekToGenerate);
    if (!week) {
      return NextResponse.json({ error: `Semana ${weekToGenerate} não gerada pela IA`, missing: missingWeeks }, { status: 500 });
    }
    ...
    const remainingMissing = missingWeeks.filter(w => w !== weekToGenerate);
