      ...
      <main className="max-w-3xl mx-auto px-6 py-8 space-y-5">
        ...
        <div className="card">
          ...
          <div className="grid grid-cols-3 gap-3 pt-5 border-t border-[#1f1f1f]">
            <div className="text-center bg-[#161616] rounded-xl py-3">
              <p className="text-2xl font-bold text-white">{session.plannedDistance ?? "—"}</p>
              <p className="text-xs text-zinc-500 mt-0.5">km planeados</p>
            </div>
            <div className="text-center bg-[#161616] rounded-xl py-3">
              <p className="text-2xl font-bold text-white">{session.plannedDuration ?? "—"}</p>
              <p className="text-xs text-zinc-500 mt-0.5">min planeados</p>
            </div>
            <div className="text-center bg-[#161616] rounded-xl py-3">
              <p className="text-2xl font-bold text-white">{session.plannedPace ?? "—"}</p>
              <p className="text-xs text-zinc-500 mt-0.5">pace alvo</p>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4">
              {session.rpe && (
                <div className="bg-blue-500/5 border border-blue-500/15 rounded-xl p-3">
                  ...
                  <p className="text-sm text-zinc-300">{session.rpe}</p>
                </div>
              )}
              {session.keyFocus && (
                <div className="bg-purple-500/5 border border-purple-500/15 rounded-xl p-3">
                  ...
                  <p className="text-sm text-zinc-300">{session.keyFocus}</p>
                </div>
              )}
            </div>
          )}
              ...
              {["z1", "z2", "z3", "z4", "z5"].map((z, i) => {
                ...
                return (
                  <div key={z} className="flex items-center gap-3">
                    <span className="text-xs font-medium text-zinc-500 w-6">Z{i + 1}</span>
                    <div className="flex-1 bg-[#1a1a1a] rounded-full h-2.5">
                      <div className={`h-2.5 rounded-full ${zoneColors[i]}`} style={{ width: `${pct}%` }} />
                    </div>
                    <span className="text-sm font-medium text-zinc-400 w-10 text-right">{pct}%</span>
                  </div>
                );
              })}
            </div>
        ...
        <div className="card space-y-5">
          ...
          {session.warmup && (
            <div className="border-l-2 border-green-500 pl-4">
              ...
              <p className="text-sm text-zinc-300 leading-relaxed">{session.warmup}</p>
            </div>
          )}
          ...
          {session.mainSet && (
            <div className="border-l-2 border-blue-500 pl-4">
              ...
              <p className="text-sm text-zinc-300 leading-relaxed">{session.mainSet}</p>
            </div>
          )}

          {session.cooldown && (
            <div className="border-l-2 border-zinc-500 pl-4">
              <p className="text-xs font-semibold text-zinc-400 uppercase tracking-wide mb-1.5">Arrefecimento</p>
              <p className="text-sm text-zinc-300 leading-relaxed">{session.cooldown}</p>
            </div>
          )}
        </div>
        ...
        {session.coachTip && (
          <div className="flex gap-3 p-4 rounded-2xl border border-yellow-500/20 bg-yellow-500/5">
            ...
            <div>
              ...
              <p className="text-sm text-zinc-300 leading-relaxed">{session.coachTip}</p>
            </div>
          </div>
        )}
        ...
        {session.activity ? (
          ...
          <div className="text-center py-8 rounded-2xl border border-dashed border-[#2a2a2a]">
            <p className="text-zinc-600 text-sm">Ainda sem atividade registada para este treino</p>
